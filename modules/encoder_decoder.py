...
def attention(query, key, value, mask=None, dropout=None):
    # query key value shape (mb, n_heads, 64)
    d_k = query.size(-1)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        for layer in self.layers:
            x = layer(x, mask) # x.shape (mb, 98, 512)
        return self.norm(x)
...
class LayerNorm(nn.Module):
    def __init__(self, features, eps=1e-6):
        ...
        self.gamma = nn.Parameter(torch.ones(features)) # (512,)
        self.beta = nn.Parameter(torch.zeros(features)) # (512,)
        self.eps = eps
        

    def forward(self, x):
        mean = x.mean(-1, keepdim=True) # (mb, 98, 1)
        std = x.std(-1, keepdim=True) # (mb, 98, 1)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, hidden_states, src_mask, tgt_mask, memory):
        ...
        return self.norm(x), hidden_states, x
...
class ConditionalLayerNorm(nn.Module):
    ...
    def forward(self, x, memory):
        ### x.shape (mb, seq_len, 512), memory.shape (mb, seq_len, n_slots * 512)
        mean = x.mean(-1, keepdim=True)
        std = x.std(-1, keepdim=True)
        delta_gamma = self.mlp_gamma(memory) ## delta_gamma (mb, seq_len, 512)
        delta_beta = self.mlp_beta(memory) ## delta_beta (mb, seq_len, 512)
        gamma_hat = self.gamma.clone() ## self.gamma (512,)
        beta_hat = self.beta.clone() ## self.beta (512,) 
        gamma_hat = torch.stack([gamma_hat] * x.size(0), dim=0) ## gamma_hat (mb, 512)
        gamma_hat = torch.stack([gamma_hat] * x.size(1), dim=1) ## gamma_hat (mb, seq_len, 512)
        beta_hat = torch.stack([beta_hat] * x.size(0), dim=0) ## beta_hat (mb, 512)
        beta_hat = torch.stack([beta_hat] * x.size(1), dim=1) ## beta_hat (mb, seq_len, 512)
...
class RelationalMemory(nn.Module):
    ...
    def init_memory(self, batch_size):
        memory = torch.stack([torch.eye(self.num_slots)] * batch_size) # memory.shape (mb, n_slots, n_slots)
        if self.d_model > self.num_slots:
            diff = self.d_model - self.num_slots
            pad = torch.zeros((batch_size, self.num_slots, diff))
            memory = torch.cat([memory, pad], -1) # memory.shape (mb, n_slots, d_model)
    ...
    def forward(self, inputs, memory):
        # inputs.shape (mb, seq_len, 512)
        outputs = []
        for i in range(inputs.shape[1]):
            memory = self.forward_step(inputs[:, i], memory) # 
            outputs.append(memory)
        outputs = torch.stack(outputs, dim=1) # (mb, seq_len, n_slots * d_model)
        return outputs
...
class EncoderDecoder(AttModel):
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None):
        ...
        # self.att_embd will project image feature to lower dimension, inherited from AttModel
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        out, src_embed, tgt_embed = self.model(att_feats, seq, att_masks, seq_mask)
        logits = F.log_softmax(self.logit(out), dim=-1)
        return logits, src_embed, tgt_embed
